...
with tab1:
    ...
    st.info(
        ...
        "櫃買指數因為 Yahoo 國際版沒有這筆資料，暫時無法顯示，可到 Yahoo奇摩股市查詢）"
    )
...
with tab4:
    st.caption(
        "彙整自《理財達人秀 EBCmoneyshow》YouTube頻道（東森財經台製作）的最新影片，"
        "優先抓取影片字幕內容進行分析，字幕抓不到才退回只用標題+說明文字。"
        "純粹整理呈現，不代表本系統認同其分析或個股推薦。節目本身也聲明「與分析師所推介個股無不當之財務利益關係，"
        "資料僅供參考」，投資決策請自行獨立判斷。"
    )

    with st.spinner("讀取理財達人秀最新影片並分析字幕內容中...（有字幕的影片會比較慢）"):
    ...
    if not videos:
        ...
        for v in videos:
            with st.container(border=True):
                ...
                source_tag = "📝 已用字幕內容分析" if v.get("used_transcript") else "⚠️ 無字幕，退回用標題+說明分析"
                st.caption(f"發布時間：{v['published']}　|　{source_tag}")
                ...
                if v["mentioned_stocks"]:
                    ...
                    st.write("🔎 內容中未偵測到明確的股票名稱或代號")

        st.caption(
            "「內容中提及的股票」優先從影片字幕全文比對，字幕抓不到才用標題+說明文字。"
